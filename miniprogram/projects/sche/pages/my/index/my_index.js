...
Page({
	...
	_getMyDate: async function (e) {
		try {

			let data = { date: timeHelper.time('Y-M-D') };
			let opts = {
				title: 'bar'
			}
			await cloudHelper.callCloudSumbit('day/day_my', data, opts).then(res => {
				this.setData({ myDay: res.data });
