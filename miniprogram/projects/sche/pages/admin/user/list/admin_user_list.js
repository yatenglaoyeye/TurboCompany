...
Page({
	...
	bindLevelTap: async function (e) {
		if (!AdminBiz.isAdmin(this)) return;
		let level = pageHelper.dataset(e, 'level');
		...
		let cb = async () => {
			try {
				await cloudHelper.callCloudSumbit('admin/user_level', params).then(res => {
 
					let dataName = 'dataList.list[' + idx + '].USER_LEVEL';
					this.setData({ 
						[dataName]: level
					});
	...
	_getSearchMenu: async function () {
		...
		let sortMenus = [
			...
			{ label: '领导', type: 'level', value: '1' },
			{ label: '员工', type: 'level', value: '0' },
		...
		for (let k = 0; k < projectSetting.DEPT_OPTIONS.length; k++) {
			sortItems2.push({ label: projectSetting.DEPT_OPTIONS[k], type: 'dept', value: projectSetting.DEPT_OPTIONS[k] });
		}
