...
function getNowTime(that) {
	...
	// 本月第几周
	let weekOfMonth = this.weekIndexInMonth(year, month, today);
	...
	else if (that.data.mode == 'continue' && that.data.continueDoDay && that.data.continueDoDay.length > 0 && that.data.continueDoDay[0]) {
		year = Number(timeHelper.timestamp2Time(timeHelper.time2Timestamp(that.data.continueDoDay[0]), 'Y'));
		month = Number(timeHelper.timestamp2Time(timeHelper.time2Timestamp(that.data.continueDoDay[0]), 'M'));
	}
	...
	let continueDoDay = that.data.continueDoDay;
...
//连选天点击
function bindDayContinueTap(e, that) {

	let getDay = e.currentTarget.dataset.fullday;

	// 显示 
	let continueDoDay = dataHelper.deepClone(that.data.continueDoDay);


	if (continueDoDay.length == 2
		&& (
			(getDay > continueDoDay[0] && getDay < continueDoDay[1]))
		|| getDay == continueDoDay[0]
		|| getDay == continueDoDay[1]
	) {
		// 区间命中
		continueDoDay = [];
	}
		...
		// 去重排序
		continueDoDay.push(getDay);
		continueDoDay = Array.from(new Set(continueDoDay));
		continueDoDay.sort();
	}
	...
	// 控制在只有2个数组 
	if (continueDoDay.length == 1)
		continueDoDay = [continueDoDay[0], continueDoDay[0]];
	else if (continueDoDay.length > 1)
		continueDoDay = [continueDoDay[0], continueDoDay[continueDoDay.length - 1]];
