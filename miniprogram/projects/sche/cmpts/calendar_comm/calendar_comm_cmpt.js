...
Component({
	...
	properties: {
		...
		continueDoDay: { // 连选模式>正在操作的天 arrary[]
			type: Array,
			value: [],
		},
	...
	methods: {
		_init: function () {
			// 今天
			let nowMonth = timeHelper.time('Y-M');
			this.setData({
				nowMonth
			})
			...
			// 连选模式
			if (this.data.mode == 'continue') {
				let continueDoDay = this.data.continueDoDay;
				continueDoDay.sort();
				continueDoDay = continueDoDay.slice(0, 2);

				if (this.data.continueDoDay.length == 1) {
		...
		bindDayOneTap(e) { // 单个天点击
			...
			console.log(this.data.selectTimeout)
			if (!this.data.selectTimeout && day < now)
				return pageHelper.showNoneToast('已过期', 1000);

			calendarLib.bindDayOneTap(e, this);
		},

		bindDayContinueTap(e) { // 连选天点击 
			// 过期时间判断
			...
			calendarLib.bindDayContinueTap(e, this);
		},
