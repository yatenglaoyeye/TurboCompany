...
class PassportService extends BaseProjectService {
	...
	async register(userId, {
		...
		let cntNo = await UserModel.count(whereNo);
		if (cntNo > 0) this.AppError('该工号已注册');
	...
	async editBase(userId, {
		...
		console.error(obj.dept)

		// 更改部门
		DayModel.edit({ DAY_USER_ID: userId }, { DAY_DEPT: obj.dept });
	...
	async login(userId) {
		...
		let fields = 'USER_ID,USER_MINI_OPENID,USER_NAME,USER_MOBILE,USER_PIC,USER_STATUS,USER_OBJ';
